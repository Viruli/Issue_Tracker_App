...
// update Issue
export const updateIssueRepo = (id: string, updatedData: Partial<Issue>) => {
  const existing = db.getFirstSync<Issue>(`SELECT * FROM issues WHERE id = ?`, [
    id,
  ]);
  ...
  db.runSync(
    ...
    [
      ...
      updatedIssue.priority ?? "Low",
      updatedIssue.assignee ?? "",
      updatedIssue.imageUri ?? "",
      id,
    ],
  );
};
